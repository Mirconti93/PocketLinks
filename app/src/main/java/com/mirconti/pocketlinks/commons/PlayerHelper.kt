...
class PlayerHelper {
    ...
    companion object {
        ...
        fun sortPlayerByRole(players: List<CategoryModel>): List<CategoryModel> {
            return players.sortedBy { it?.role }
        }

        val comparator = { p1: CategoryModel?, p2: CategoryModel? ->
            when {
                (p1 == null && p2 == null) -> 0
                (p1 == null) -> -1
                else -> {
        ...
        fun getBestInRole(list: List<CategoryModel>, role: Enums.Role, isLegend: Boolean): CategoryModel? {
                ...
                if (role == p.role) {
                    if (categoryModel != null) {
                        ...
                        val playerModelVal: Int =
                            if (isLegend) categoryModel.valueleg?.toInt() ?: 0 else categoryModel.value?.toInt() ?: 0
            ...
            if (categoryModel == null) {
                categoryModel = getBestPlayer(list, isLegend)
            }
            return categoryModel
        }

        fun getBestPlayer(categoryModels: List<CategoryModel>?, isLegend: Boolean): CategoryModel? {
            if (categoryModels == null || categoryModels.isEmpty()) {
                return null
            }
            var playerModel = categoryModels[0]
            for (p in categoryModels) {
                val pVal: Int = if (isLegend) p.valueleg?.toInt() ?: 0  else p.value?.toInt() ?: 0
                val playerModelVal: Int =
        ...
        fun buildPlayerModel(name: String): CategoryModel {
            return CategoryModel(name, Enums.Role.PP, null, null, null, null, null, null, null, null, null, Enums.RoleLineUp.PPM, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50)
        }
